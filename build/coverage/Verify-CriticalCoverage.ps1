...
<#
.SYNOPSIS
    Verifies that each tested MyAvalonia assembly meets the configured line coverage threshold.

.DESCRIPTION
    Reads Cobertura files under TestResults/coverage/{Project}.Tests/ and checks the primary
    assembly (MyAvalonia.X.Tests -> MyAvalonia.X) against assembly-thresholds.json.
#>
param(
    [string] $CoverageRoot = '',
...
$testProjects = Get-ChildItem -LiteralPath $TestsRoot -Directory |
    Where-Object { $_.Name -like 'MyNet.Avalonia.*.Tests' }
