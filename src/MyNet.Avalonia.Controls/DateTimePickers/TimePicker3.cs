// -----------------------------------------------------------------------
// <copyright file="TimePicker3.cs" company="Stéphane ANDRE">
...
public class TimePicker3 : TimePickerBase
{
    ...
    public static readonly StyledProperty<TimeSpan?> SelectedTimeProperty = AvaloniaProperty.Register<TimePicker3, TimeSpan?>(nameof(SelectedTime), defaultBindingMode: BindingMode.TwoWay);
    ...
    static TimePicker3()
    {
        FocusableProperty.OverrideDefaultValue<TimePicker3>(true);
        _ = SelectedTimeProperty.Changed.AddClassHandler<TimePicker3, TimeSpan?>((picker, args) =>
            picker.OnSelectionChanged(args));
        _ = DisplayFormatProperty.Changed.AddClassHandler<TimePicker3, string?>((picker, _) =>
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        _presenter?.MoveToTime(SelectedTime);
        SyncTimeToText(SelectedTime);
        SyncTimeToText(SelectedTime);
    }
    ...
    private void OnTextChanged(object? sender, TextChangedEventArgs e)
    {
        ...
        else if (string.IsNullOrEmpty(DisplayFormat))
        {
            ...
            {
                _presenter?.MoveToTime(defaultTime);
            }
        }
        else
        {
    ...
    private void OnSelectionChanged(AvaloniaPropertyChangedEventArgs<TimeSpan?> args)
    {
        ...
        _presenter?.MoveToTime(args.NewValue.Value);
    ...
    private void CommitInput(bool clearWhenInvalid)
    {
        if (DateTime.TryParseExact(_textBox?.Text, DisplayFormat, CultureInfo.CurrentCulture, DateTimeStyles.None, out var time))
        {
            ...
            _presenter?.MoveToTime(time.TimeOfDay);
        }
        else
        {
            if (clearWhenInvalid)
