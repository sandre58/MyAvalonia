...
public class DateTimeScrollPickerEx : DatePicker
{
    /// <summary>
    ...
    public static readonly StyledProperty<TimeSpan?> SelectedTimeProperty =
        AvaloniaProperty.Register<DateTimeScrollPickerEx, TimeSpan?>(nameof(SelectedTime), defaultBindingMode: BindingMode.TwoWay, enableDataValidation: true);
    ...
    public DateTimeScrollPickerEx()
    {
        ...
        if (timePattern.Contains('H', StringComparison.OrdinalIgnoreCase))
    ...
    /// <summary>
    /// Gets or sets a value indicating whether use seconds switch, either true or false.
    /// </summary>
    ...
    private void SetSelectedTimeText()
    {
        ...
        if (time.HasValue)
        {
            ...
            if (ClockIdentifier == "12HourClock")
            {
                ...
                newTime = new(hr, newTime.Minutes, newTime.Seconds);
            }

            _hourText.Text = newTime.ToString("%h", CultureInfo.CurrentCulture);
            _minuteText.Text = newTime.ToString("mm", CultureInfo.CurrentCulture);
            if (_secondText is not null)
            {
                _secondText.Text = newTime.ToString("ss", CultureInfo.CurrentCulture);
            }
    ...
    protected virtual void OnSelectedTimeChanged(TimeSpan? oldTime, TimeSpan? newTime) => SelectedTimeChanged?.Invoke(this, new(oldTime, newTime));
