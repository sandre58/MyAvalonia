# Contributing to MyAvalonia

Thank you for your interest in contributing to **MyAvalonia** (My .NET). This monorepo publishes modular NuGet packages for MVVM desktop applications on **.NET 10**.

## Before you start

- Read the [documentation index](docs/index.md) and [getting started](docs/getting-started.md) guide.
- Search [existing issues](https://github.com/sandre58/MyAvalonia/issues) to avoid duplicates.
...
- Add or update unit tests in the matching `tests/MyAvalonia.*.Tests` project.
...
- Package `README.md` under `src/MyAvalonia.*/`
...
All **4 MyAvalonia NuGet packages** share a **single version** (lockstep monorepo). Version numbers are **not** edited in `.csproj` files.
