...
internal sealed class CardPageViewModel(ICommandFactory commands, INotificationPublisher notificationPublisher) : ShowcaseViewModel(nameof(Card), commands, [
    new ControlThemeBuilder()
        ...
        .AddAllRoles()
        .AddDefaultSizes()
        .AddEnumProperty<CardLayout, ComboBoxEditor>(Card.CardLayoutProperty, configure: x => x.DisplayName(nameof(SettingsResources.Layout)))
        .AddEnumProperty<LeadingPresentation, ListBoxEditor>(Card.LeadingPresentationProperty, configure: x => x.DisplayName(nameof(SettingsResources.LeadingPresentation)))
